...
COPY public.messages (id, content, sender, user_receiver, group_receiver, original_message, sender_id, sent_at) FROM stdin;
\.
...
CREATE TABLE public.messages (
    ...
    sender_id integer,
    sent_at date DEFAULT now() NOT NULL,
    status public.message_status DEFAULT 'UNSEEN'::public.message_status
);
...
COPY public.messages (id, content, sender, user_receiver, group_receiver, original_message, sender_id, sent_at, status) FROM stdin;
\.
